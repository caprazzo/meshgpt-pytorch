...
class MeshDataset(Dataset):
    ...
    Attributes:
        data (list): A list of mesh data entries. Each entry is a dictionary containing the following keys:
            ...
            texts (str, optional): A string containing the associated text information for the mesh.
            text_embeds (torch.Tensor, optional): A tensor of text embeddings for the mesh.
            images (torch.Tensor, optional): A tensor representing an image for the mesh.
            image_embeds (torch.Tensor, optional): A tensor of image embeddings for the mesh.
    ...
    ```
    data = [
        {'vertices': torch.tensor([[1, 2, 3], [4, 5, 6]], dtype=torch.float32), 'faces': torch.tensor([[0, 1, 2]], dtype=torch.long), 'texts': 'table', 'images': torch.randn(3, 224, 224)},
        {'vertices': torch.tensor([[10, 20, 30], [40, 50, 60]], dtype=torch.float32), 'faces': torch.tensor([[1, 2, 0]], dtype=torch.long), "texts": "chair"},
    ]
    ...
    def sort_dataset_keys(self):
        desired_order = ['vertices', 'faces', 'face_edges', 'texts', 'text_embeds', 'images', 'image_embeds', 'codes']
        self.data = [
    ...
    def embed_images(self, conditioner: MultiModalEmbeddingReturner, batch_size=25):
        ...
        self.sort_dataset_keys()
        print(f"[MeshDataset] Generated {len(data_to_process)} image_embeddings")